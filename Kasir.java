...
public class Kasir {
    ...
    private static void lihatTotalTransaksi() {
        ...
        for (int i = 0; i < indeksTransaksi; i++) {
            System.out.println("Transaksi ke-" + (i + 1) + ": Rp." + totalTransaksi[i] + ",-");
        }
            
        int total = getTotalTransaksi();
        System.out.println("Total Keseluruhan: Rp." + total + ",-");
    }
    
    private static int getTotalTransaksi() {
        // Hitung total transaksi dengan menjumlahkan semua transaksi dalam array
        int total = 0;
        for (int i = 0; i < indeksTransaksi; i++) {
...
System.out.println();
  ...
  System.out.println();
    ...
    // Hitung total transaksi pada pembayaran
    totalTransaksiPembayaran += total;

    // Simpan total transaksi pada array totalTransaksi
    totalTransaksi[indeksTransaksi++] = totalTransaksiPembayaran;
